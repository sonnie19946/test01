...
✅【提取标准】—— 只提取**在剧情中出现不止一次、或对剧情转折起关键作用**的道具：
  - 贯穿多个场景、角色反复使用或互动的标志性物件（如：主角的标志武器、反复出现的信物）
  - 承载象征意义或直接推动剧情转折的关键物品（如：钥匙打开了密室、信件揭露了真相）
  - 角色之间通过此物件产生重要互动且该物件在后续剧情中再次出现的道具
  - 判断标准：**如果把这个道具从剧本中删除，会影响至少两个以上的场景或剧情节点**

❌【严禁提取】：
  - **服装配饰**：任何穿戴在角色身上的衣物（外套、靴子、手套、围巾等）→ 归属角色形象节点
  - **建筑与固定设施**：路灯、桥梁、水泵、招牌等环境固有物 → 归属场景节点
  - **一次性小道具**：只出现一次、角色顺手使用即弃的物品（一块碱皂、一杯水、一根火柴、几枚硬币等）
  - **泛指食物/饮品**：除非该食物是剧情反复出现的象征物（如每天共享的那块黑面包），否则不提取

【质量底线】：宁精勿滥。3-5 个真正贯穿剧情的核心道具，远胜 20 个只出现一次的废品。`
  }
