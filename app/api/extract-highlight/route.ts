...
你在电影工业管线中的职责是：从剧本中**全面、细致**地提取出所有对叙事、角色互动、或视觉呈现有意义的物件，为概念设计师提供精确的工业级参考数据。
...

{
  "props": [
    {
      "name": "道具名称（简洁精准，如：敛尸推车、皇家猫食碗、半块黑面包）",
      "desc": "剧本原文描述的忠实复述 + 基于上下文的合理推演",
      "isActual": true,
      "appearance": {
...
第一级 ✅【强制提取】叙事核心道具：
  - 贯穿剧情主线、角色反复使用或互动的标志性物件
  - 承载象征意义或推动剧情转折的关键物品（凶器、信件、钥匙、药瓶等）
  - 角色之间通过此物件产生重要互动的道具（如：递出的面包、交换的金币）
  - 对话中被特别提及或强调的物品

第二级 ✅【尽量提取】互动型道具与视觉道具：
  - 角色主动拿起、使用、递出、丢弃的具体物品（食物、工具、容器、武器等）
  - 虽然只出现一次但具有强烈视觉冲击力或时代特征的物件
  - 人物随身携带、暗示其身份地位的小物件（烟斗、怀表、徽章等）
  - 从角色动作中可以合理推断出的隐含道具（如"她裁剪布料"→ 剪刀和布料）

第三级 ❌【严禁提取】：
  - 建筑固有设施（墙壁、门窗、楼梯、路灯）→ 归属场景节点
  - 穿戴在角色身上的服装配饰 → 归属角色形象节点
  - 完全无法识别、无具体描述、纯粹作为背景模糊存在的物品

【全面性铁律】：你的目标是**宁多勿漏**。如果犹豫某物件是否应该提取，那就提取它。遗漏一个有价值的道具远比多提取一个无用道具更严重。
- 仔细扫描剧本中的每一个动词（拿起、放下、递出、打开、砸碎……），每个动词背后几乎都隐含着一个可提取的道具。
- 食物、饮品、货币等消耗品，只要角色有明确互动，一律提取。
- 动物（猫、狗、马等）如果在剧本中扮演重要角色或有大量镜头，也应作为特殊道具提取。`
  }
...

{
  "scenes": [
    {
      "name": "场景名称（简洁精准，如：贫民窟小巷、皇宫猫舍、河岸垃圾堆）",
      ...
      "staticVisualAnchors": "静态视觉锚点/环境陈设（剥离活物后，环境中最抓眼的死物，如：巷道尽头一座歪斜的生锈铁炉、墙角堆积的成堆腐烂垃圾）",
      "colorPalette": "色彩方案与分布（如：大面积冷灰与暗青色主导，建筑阴影呈深蓝紫色，局部有锈红色微弱点缀）"
    }
  ]
}
...
1. **显式场景必提**：剧本中明确出现的地点名词（街道、酒馆、广场、宫殿、桥洞下……）全部提取，无一遗漏。

2. **隐含场景必推**：角色的动作暗示了场景，即使剧本没有写明地名也必须提取并合理命名。
   - "他们蜷缩在桥下过夜" → 提取场景"桥洞下的临时营地"
   - "她跑进一家面包店" → 提取场景"街角面包店"
   - "审判在众人面前进行" → 提取场景"公共审判广场"

3. **场景变体拆分**：如果同一个地点在不同时间段呈现截然不同的视觉状态（白天 vs 深夜、暴风雨前 vs 暴风雨后），应拆分为独立场景。

4. **绝对无人空镜**：场景 = 开拍前的空舞台。严禁写入任何人物、动物或动态事件。所有生物活动必须剥离。

5. **五感视觉化**：你只有一台摄影机，没有麦克风和鼻子。
   - "喧闹的" → "墙面贴满层层叠叠的泛黄告示，地面散落被踩碎的纸屑"
   - "恶臭的" → "排水沟口覆盖着一层泛绿的粘稠膜状物"
   - "寒冷的" → "所有金属表面凝结了一层薄薄的白色霜花"

6. **时代考古学**：根据剧本推断的时代背景，为建筑风格、材料质感、光源类型做强制校准。
   - 19世纪 → 煤气灯/烛光/砖石结构
   - 二战时期 → 混凝土碉堡/电灯泡/军用铁丝网
   - 近未来 → 全息投影/纳米涂层/环境自适应照明

7. **构图思维**：每个字段都应暗含"如果我站在这个空间中央，向前看会看到什么"的构图意识。

【全面性铁律】：你的目标是**宁多勿漏**。一部电影通常有 8-20 个独立场景。仔细扫描剧本中每一次空间转换（室内→室外、A地→B地、楼上→楼下），每次转换几乎都意味着一个新场景。`
  }
